...
declare -A SERVER_ENV=(
  ...
  [WEB_UI_CONFIG_FILE]="${WEB_UI_CONFIG_FILE:-${REPO_ROOT}/tests/config/ci/ocis-config.json}"
...
# Build env for the subprocess (after all SERVER_ENV mutations are complete)
ENV_ARGS=()
for key in "${!SERVER_ENV[@]}"; do
